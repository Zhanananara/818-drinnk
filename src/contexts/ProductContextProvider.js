...
import { ACTIONS, API1, PRODUCTS_LIMIT } from "../helpers/consts";
...
const ProductContextProvider = ({ children }) => {
  ...
  const getProducts = async () => {
    try {
      let res = await axios.get(`${API1}${window.location.search}`);
      // console.log(res);
      dispatch({
  ...
  const addProduct = async (newProduct) => {
    try {
      let res = await axios.post(API1, newProduct);
  ...
  const deleteProduct = async (prod) => {
    try {
      let res = await axios.delete(`${API1}/${prod.id}`);
  ...
  const saveEditedProd = async (editedProd) => {
    try {
      let res = await axios.patch(`${API1}/${editedProd.id}`, editedProd);
      getProducts();
    } catch (err) {
      notifyError(err);
    }
